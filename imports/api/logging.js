...
var recordParticipantsBatchId = function() {
    var participantsBatchAssignment = "";
    for ( var i = 0; i < Participants.participantsQueue.length; i++) {
        var _id = Participants.participantsQueue[i];
        participantsBatchAssignment += Participants.id_name[_id] + "\t" + _id + "\t" + Participants.id_batch[_id] + "\n";
    }
    // Remove the last "\n" character
    participantsBatchAssignment = participantsBatchAssignment.slice(0, participantsBatchAssignment.length-1);

    // insert into experimental log
    insertExperimentLogEntry("PBID1", participantsBatchAssignment);
}